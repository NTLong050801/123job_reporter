...
@stop
@section('content')
    {!! Breadcrumbs::render('subscribe-overview')!!}
    <div class="row">
        <div class="col-md-12">
            <div id="table-list">
                    ...
                    <div class="box-body">
                        <div class="row-employee data-grid-header clearfix">
                            <div class="button-groups pull-left">
                                <div class="btn-group">
                                    <a id="assign" href="{{ route('get.subscribe-job.overview') }}" class="btn btn-primary btn-sm">
                                        <i class="fa fa-eye"></i> Xem theo ngày
                                    </a>
                                </div>
                                <div class="btn-group">
                                    <a id="assign" href="{{ route('get.subscribe-job.overview_month') }}" class="btn btn-default btn-sm">
                        ...
                        <table class="table table-hover table-bordered table-scroll">
                            ...
                            <tbody>
                            @php $count = 0; @endphp
                            @foreach($items['itemDateArr'] as $date => $itemDate)
                                <tr>
                                    <td class="text-center">{{ ++$count }}</td>
                                    <td>{{ $date }}</td>

                                    @foreach($itemDate['visitor'] as $valueVisitor)
                                    <td class="text-center">
                                        <a href="" class="text-primary"><u>{{ $valueVisitor }}</u></a>
                                    </td>
                                    @endforeach
                                    <td class="text-center">
                                        <a href="" class="text-success"><u>{{ $itemDate['percent'] }} %</u></a>
                                    </td>
                                </tr>
                            @endforeach
