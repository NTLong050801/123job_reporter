                    ...
                    <table class="table table-bordered table-hover">
                        <thead>
                            <tr>
                                ...
                                <th width="7%" class="text-center">Ngày vào|Kết thúc</th>
                            ...
                            @foreach ($users as $key => $user)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <p class="label label-default">{{ $user->department->name ?? 'N\A' }}</p>
                                    </td>
                                    ...
                                    <td align="center">
                                        @if(can('get.employee.edit') or isAdmin())
                                        <a href="{{ route('get.employee.edit', $user->id) }}" title="Chỉnh sửa"
                                            class="btn btn-sm btn-primary edit"><i class="fa fa-pencil"></i></a>
                                         @endif
                                         @if(can('get.employee.edit_pwd') or isAdmin())
                                            <a href="{{ route('get.employee.edit_pwd', $user->id) }}"
                                               title="Thay đổi mật khẩu" class="btn btn-sm btn-success change-pwd"><i
                                                    class="fa fa-cog"></i></a>
                                         @endif
                                        {{-- @if ($current_user->roles[0]->id == 1) --}}
                                        @if(isAdmin())
                                            <a href="{{ route('get.employee.fake_login', $user->id) }}"
                                               title="Fake login" class="btn btn-sm btn-warning change-pwd"><i
