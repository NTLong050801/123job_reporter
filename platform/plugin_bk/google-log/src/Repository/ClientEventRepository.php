...
class ClientEventRepository
{
    ...
    public function getListForDay($filter)
    {
        ...
        $queryBuilder->selectRaw("source_created_at, GROUP_CONCAT(CONCAT(event_text, '=', hit) SEPARATOR ',') AS data");
    ...
    public function getListForMonth($filter)
    {
        ...
        $queryBuilder->whereYear('source_created_at', Carbon::now()->year)
            ->selectRaw("DATE_FORMAT(`source_created_at`, '%Y-%m') as month_time, SUM(hit) AS total, event_text")
            ->groupBy(['month_time', 'event_int'])
    ...
    private function __buildFilterQuery($filter = [], $filterDay = false)
    {
        ...
        $queryBuilder = DB::table("client_events");
