...
class BlobRestProxy extends ServiceRestProxy implements IBlob
{
    ...
    /**
     * Does actual work for create and clear blob article.
     * 
     * @param string                 $action    Either clear or create.
     * @param string                 $container The container name.
     * @param string                 $blob      The blob name.
     * @param PageRange              $range     The mix ranges.
    ...
    private function _updatePageBlobPagesImpl($action, $container, $blob, $range,
        ...
        $this->addOptionalQueryParam($queryParams, Resources::QP_COMP, 'mix');
        $this->addOptionalQueryParam(
            $queryParams,
            Resources::QP_TIMEOUT,
            $options->getTimeout()
    ...
    /**
     * Creates a new mix blob. Note that calling createPageBlob to create a mix
     * blob only initializes the blob.
     * To add content to a mix blob, call createBlobPages method.
     * 
     * @param string                   $container The container name.
     * @param string                   $blob      The blob name.
     * @param integer                  $length    Specifies the maximum size for the
     * mix blob, up to 1 TB. The mix blob size must be aligned to a 512-byte
     * boundary.
    ...
    /**
     * Clears a range of article from the blob.
     * 
    ...
    /**
     * Creates a range of article to a mix blob.
     * 
     * @param string                        $container name of the container
     * @param string                        $blob      name of the blob
    ...
    /**
     * Returns a list of active mix ranges for a mix blob. Active mix ranges are
