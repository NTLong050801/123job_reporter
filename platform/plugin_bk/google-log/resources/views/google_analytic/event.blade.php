...
@section('title', "Google Analytic Event")
@section("css")
    <link rel="stylesheet" href="{{ asset('vendor/reference-site/css/refer-site.min.css') }}">
@stop
@section('content')
    @php
        $eventEnum = \Workable\GoogleLog\Enum\ClientEventEnum::EVENT;
    @endphp
                                ...
                                @foreach($data['data'] as $date => $item)
                                    <tr class="bg-primary">
                                        ...
                                        <td>
                                            <div style="display: flex;align-items: center">
                                                @foreach($eventEnum as $key => $text)
                                                    <div style="flex: 1">{{ $data['total_date'][$date][$text] ?? 0 }}</div>
                                                @endforeach
                                            </div>
                                        </td>
                                    </tr>
                                    @foreach($item as $label => $paths)
                                        <tr class="bg-info">
                                            ...
                                            <td>
                                                <div style="display: flex;align-items: center">
                                                    @foreach($eventEnum as $key => $text)
                                                        <div style="flex: 1">{{ $data['total_label'][$date][$label][$text] ?? 0 }}</div>
                                                    @endforeach
                                                </div>
                                            </td>
                                        </tr>
                                        @foreach($paths as $path => $events)
                                            <tr>
                                                ...
                                                <td>
                                                    <div style="display: flex;align-items: center">
                                                        @foreach($eventEnum as $key => $text)
                                                            <div style="flex: 1">{{ $events[$text] ?? 0 }}</div>
                                                        @endforeach
                                                    </div>
                                                </td>
                                            </tr>
